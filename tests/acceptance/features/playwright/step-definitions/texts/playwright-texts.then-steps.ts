...
Then(/^the text "(?<text>.+?)" under the (?<role>button|img|heading|navigation|link|region|alertdialog|dialog) with name "(?<name>.+)" should be visible$/u, async function(this: CustomWorld, text: string, role: LocatorRole, name: string): Promise<void> {
