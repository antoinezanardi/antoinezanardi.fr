...
When(/^the user clicks on the (?<role>button|link|menuitem|tab|img) with(?<isExact> exact)? name "(?<name>.+?)"$/u, async function(this: CustomWorld, role: LocatorRole, exact: string | null, name: string): Promise<void> {
  await clickOnRoleWithText(this, role, name, exact !== null);
});

When(/^the user hovers the (?<role>button|link|menuitem|tab|img|alertdialog) with(?<isExact> exact)? name "(?<name>.+?)"$/u, async function(this: CustomWorld, role: LocatorRole, exact: string | null, name: string): Promise<void> {
